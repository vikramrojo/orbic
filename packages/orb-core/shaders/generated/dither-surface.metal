...
float ditherNoise(float2 p) {
    ...
    float b = ditherHash(i + float2(1.0, 0.0));
    float c = ditherHash(i + float2(0.0, 1.0));
    float d = ditherHash(i + float2(1.0, 1.0));
...
float3 field(float2 p, float t, float energy, float coherence, float warmth, float pulse) {
    ...
    float signal = ditherNoise(p * 3.1 + float2(clock * 0.09, clock * -0.06)) * 0.75
                 + ditherNoise(p * 6.7 - float2(clock * 0.05, clock * 0.08)) * 0.25;
    ...
    float3 cool = float3(0.30, 0.46, 0.76);
    float3 warm = float3(0.86, 0.52, 0.26);
    ...
    float3 col = tint * quantised * 1.15 * amplitude;
