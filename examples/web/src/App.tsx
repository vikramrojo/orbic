...
import { FIELD_NAMES, PRESET_NAMES } from '@orbic/core';
import type { PresetName } from '@orbic/core';

/**
 * Fields the harness offers, derived from the build rather than hardcoded.
 *
 * `FIELD_NAMES` is generated by `scripts/build-shaders.mjs` from whatever
 * `.orb` files are in `packages/orb-core/shaders/fields/`, so a custom field
 * dropped in there appears here — in the switchers, the gallery, and the
 * playground — with no edit to this file. That is the point: the harness has
 * to be able to field ANY number of shader builds, not the three it happened
 * to ship with. See docs/custom-fields.md.
 *
 * `flat-color` is excluded on purpose: it is the pipeline-proof placeholder,
 * not a field worth judging aesthetically. It is the one name filtered here,
 * and it is filtered by name rather than by position so that adding fields
 * cannot accidentally reorder it back in.
 */
const PLACEHOLDER_FIELD = 'flat-color';
const REVIEW_FIELDS: readonly string[] = FIELD_NAMES.filter((f) => f !== PLACEHOLDER_FIELD);
type Field = string;
...
/**
 * Usage snippets are generated from the CURRENT selection rather than written
 * out with a fixed field name, so the code shown is the code for the orb you
 * are actually looking at — including a custom field the build picked up,
 * which a hardcoded example could never mention.
 */
function usageSnippet(platform: Platform, field: string, state: PresetName): string {
  switch (platform) {
    case 'swiftui':
      return `import Orbic\n\nOrb(field: "${field}", state: "${state}")`;
    case 'react-native':
      return `import { Orb } from '@orbic/native';\n\n<Orb field="${field}" state="${state}" />`;
    default:
      return `import { Orb } from '@orbic/web';\n\n<Orb field="${field}" state="${state}" />`;
  }
}
...
export function App() {
  ...
  const gallery = useMemo(() => buildGallery(REVIEW_FIELDS), []);
  const [field, setField] = useState<Field>(REVIEW_FIELDS[0]!);
  ...
  const [pgField, setPgField] = useState<Field>(REVIEW_FIELDS[REVIEW_FIELDS.length - 1]!);
      ...
      <div className="orbic-shell">
        ...
        <header className="orbic-header">
          <div className="orbic-tile">
            <Orb field={REVIEW_FIELDS[0]} state="subtle" size={20} />
          </div>
        ...
        <section className="orbic-section">
          ...
          <CodeBlock id="usage" code={usageSnippet(platform, pgField, pgState)} copiedId={copiedId} onCopy={copy} />
        </section>
