...
**The gate is not yet closed.** All six fields analysed come from pbakaus/radiant and therefore share uniform conventions, so they do not exercise the contract independently. Task 3.5 requires a field from an unrelated source, and the ABI is not frozen until that passes.
