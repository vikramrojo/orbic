...
- [ ] 9.2 BLOCKER FOR ALL SWIFT WORK — resolve how SPM consumes `.metal`. `swift build` warns "found 2 file(s) which are unhandled" for `Sources/Orbic/Shaders/*.metal`: a pure Swift target does not compile Metal sources by default, and the local toolchain has no Metal compiler to do it anyway. Determine empirically whether SwiftPM compiles them into a `default.metallib` under full Xcode (loadable via `ShaderLibrary.bundle(.module)`), or whether they must be precompiled to `.metallib` in CI and shipped as a resource. Do not guess — this decides the Swift distribution model
