...
- [x] 10.1 Configure per-field entry points and assert an unused field is absent from a consumer bundle — the generator now emits one module per field instead of a single `FIELD_SHADERS` object (bundlers cannot drop unused PROPERTIES of an object literal, so the barrel shipped every field to every consumer). `@orbic/web/minimal` + `@orbic/web/fields/*` let a consumer register only what they use; the main entry still registers everything. Asserted by BUNDLING with esbuild, not by reading imports — static analysis would not have caught the barrel problem, since the import genuinely is one module
- [x] 10.2 Measure and record shipped bundle size per platform for a one-field consumer — 53.1 kB for one field vs 190.6 kB for all five (minified ESM, React external), recorded in README.md along with per-field gzipped payloads. The README's previous figures (1.3–2.7 kB) were wrong: each field module carries both shapes and each artifact embeds the full preamble/compositor, so the shared parts are duplicated per field rather than shared — stated in the README rather than smoothed over
